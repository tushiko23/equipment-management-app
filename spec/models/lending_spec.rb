...
RSpec.describe Lending, type: :model do
  ...
  describe "返却ステータスの連動テスト" do
    it "返却日(returned_at)が記録されたら、紐づくアイテムが「貸出可」に戻ること" do
      ...
      user = User.create!(email: "test@example.com", password: "password123", name: "テスト太郎")
      item = Item.create!(name: "MacBook", unique_id: "999", category: category, state: :available)
  ...
  it "貸出中のアイテムに対しては、新しく貸出データを登録できないこと" do
    category = Category.create!(name: "備品")
    user = User.create!(email: "other@example.com", password: random_password, name: "別の人")

    lent_item = Item.create!(name: "既存の備品", unique_id: "X01", category: category, state: :borrowed)
